...
// The panel mounts fresh on every open, so search state resets naturally.
import { useState, useMemo, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useOS, useDesktop } from '@/state/os/providers';
import { sounds } from '@/lib/audio/cues';
import { appIcons, Search, Moon, Power } from '@/ui/icons/icon-map';
...
// ─── Panel — mounts fresh on every open, owns its own search state ──
    ...
    <>
      ...
      >
        ...
        <div className="p-5 pb-3">
          ...
          <div className="mt-3 flex items-center gap-2 rounded-lg px-3 py-2 bg-white/[0.06]">
        ...
        {/* Pinned Apps (when not searching) */}
        {!query && (
...
// ─── Wrapper — gates mounting so state always starts clean ──
...
export default function StartMenu({ isOpen, onClose }: StartMenuProps) {
  return (
    <AnimatePresence>
      {isOpen && <StartMenuPanel key="startmenu" onClose={onClose} />}
    </AnimatePresence>
  );
}
