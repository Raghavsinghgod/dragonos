...
export default function BootSequence() {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#050508] cursor-pointer"
        onClick={skip}
        exit={{ opacity: 0 }}
          ...
          {phase === 'spinner' && (
            ...
            >
              <div className="relative w-10 h-10">
                <motion.div
                  className="absolute inset-0 rounded-full border-2 border-transparent border-t-[#dc2626]"
                  animate={{ rotate: 360 }}
                  transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
                />
                <motion.div
                  className="absolute inset-1 rounded-full border-2 border-transparent border-t-[#dc2626]/50"
                  animate={{ rotate: -360 }}
        ...
        <AnimatePresence>
          {showSkip && (
            <motion.p
              ...
              animate={{ opacity: 0.5 }}
              exit={{ opacity: 0 }}
              className="absolute bottom-12 text-xs text-white/40 font-inter tracking-wider"
            >
